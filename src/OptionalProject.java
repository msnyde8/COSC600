...
public class OptionalProject {
	public static void main(String[] args) {
		...
		do
		{
			System.out.println("Would you like to input from: 1) Command Line 2) GUI or 0) Exit?");
			...
			try
			{
				...
				Integer optionInt = Integer.parseInt(tmpStr);
				switch(optionInt)
				{
					case 1:
...
class InfixToPostfix {
	...
	static String convertStr(String inStr)
	{
		try
		{
			...
			StringTokenizer strToken = new StringTokenizer(infixStr,"^*x/+-–() \t",true);
...
*/
			...
			do
			{
				String charStr = lineReader.nextLine();
		
				if(true == charStr.equals("end"))
				{
					break;
				}
				if((null != charStr) && (false == charStr.isEmpty()))
				{
//					System.out.println(charStr);
					System.out.println(InfixToPostfix.convertStr(charStr));
				}
				System.out.println("Please input infix expression or type 'end' to exit:");
			}while(lineReader.hasNextLine());
			lineReader.close();
		}
		catch(EmptyStackException|IllegalStateException|
